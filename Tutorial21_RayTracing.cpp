...
void Tutorial21_RayTracing::CreateRayTracingPSO()
{
    ...
    // Glass spheres will use the glass shader
    PSOCreateInfo.AddProceduralHitShader("GlassSpherePrimaryHit", pSphereIntersection, pGlassPrimaryHit);
...
void Tutorial21_RayTracing::CreateProceduralBLAS()
{
    ...
    // Create boxes for spheres - 2 large ones for glass spheres and multiple smaller ones
    HLSL::BoxAttribs Boxes[NumSpheres];

    // Create medium-sized boxes for glass spheres (smaller than original)
    Boxes[0] = HLSL::BoxAttribs{-3.0f, -3.0f, -3.0f, 3.0f, 3.0f, 3.0f}; // Medium glass sphere 1
    Boxes[1] = HLSL::BoxAttribs{-2.5f, -2.5f, -2.5f, 2.5f, 2.5f, 2.5f}; // Medium glass sphere 2

    // Create smaller boxes for regular spheres
    float smallSize = 1.0f;
    for (int i = 2; i < NumSpheres; i++)
    {
        Boxes[i] = HLSL::BoxAttribs{
    ...
    // Create & build bottom level acceleration structure
    {
        ...
        BLASBoundingBoxDesc BoxInfo;
        {
            ...
            BoxInfo.MaxBoxCount  = NumSpheres;
...
void Tutorial21_RayTracing::UpdateTLAS()
{
    ...
    static constexpr int NumTotalInstances = 1 + NumSpheres; // Ground + Spheres
    ...
    if (!m_pTLAS)
    {
        ...
        TLASDesc.Name             = "TLAS";
        TLASDesc.MaxInstanceCount = NumTotalInstances;
    ...
    if (!m_InstanceBuffer)
    {
        ...
        BuffDesc.Usage     = USAGE_DEFAULT;
        BuffDesc.BindFlags = BIND_RAY_TRACING;
        BuffDesc.Size      = TLAS_INSTANCE_DATA_SIZE * NumTotalInstances;
    ...
    // Setup instances
    TLASBuildInstanceData Instances[NumTotalInstances] = {};
    ...
    // Two medium glass spheres - positioned to touch the ground
    Instances[1].InstanceName = "Glass Sphere 1";
    Instances[1].CustomId     = 0; // box index
    Instances[1].pBLAS        = m_pProceduralBLAS;
    Instances[1].Mask         = TRANSPARENT_GEOM_MASK;
    // La primera esfera de vidrio tiene un BoxAttribs de {-3.0f, -3.0f, -3.0f, 3.0f, 3.0f, 3.0f}
    // Por lo tanto su radio es 3.0f, ajustando para que solo toque el piso
    Instances[1].Transform.SetTranslation(-9.0f, -3.0f + 0.05f, -3.0f);

    Instances[2].InstanceName = "Glass Sphere 2";
    Instances[2].CustomId     = 1; // box index
    Instances[2].pBLAS        = m_pProceduralBLAS;
    Instances[2].Mask         = TRANSPARENT_GEOM_MASK;
    // La segunda esfera de vidrio tiene un BoxAttribs de {-2.5f, -2.5f, -2.5f, 2.5f, 2.5f, 2.5f}
    // Por lo tanto su radio es 2.5f, ajustando para que solo toque el piso
    Instances[2].Transform.SetTranslation(9.0f, -3.5f + 0.05f, -3.0f);

    // Multiple smaller spheres with random positioning but touching the ground
    static const char* SphereNames[] = {
        "Small Sphere 1", "Small Sphere 2", "Small Sphere 3", "Small Sphere 4", "Small Sphere 5",
        "Small Sphere 6", "Small Sphere 7", "Small Sphere 8", "Small Sphere 9", "Small Sphere 10",
        "Small Sphere 11", "Small Sphere 12", "Small Sphere 13", "Small Sphere 14"};

    // El piso está en y = -6.0f
    float floorY = -6.0f;

    for (int i = 0; i < NumSpheres - 2; i++)
    {
        int instanceIdx = 3 + i;
        int sphereIdx   = i + 2; // Box index (after the 2 glass spheres)

        Instances[instanceIdx].InstanceName = SphereNames[i];
        Instances[instanceIdx].CustomId     = sphereIdx; // box index
        Instances[instanceIdx].pBLAS        = m_pProceduralBLAS;
        Instances[instanceIdx].Mask         = OPAQUE_GEOM_MASK;

        // Use deterministic pseudo-random positioning for x and z coordinates
        float hash1 = sin(i * 12.9898f + 4.1414f) * 43758.5453f;
        float hash2 = sin(i * 39.3465f + 7.9821f) * 43758.5453f;

        // Extract fractional parts for position values in [-20, 20] range
        float x = (hash1 - floor(hash1) - 0.5f) * 40.0f;
        float z = (hash2 - floor(hash2) - 0.5f) * 40.0f;

        // El radio de las esferas pequeñas es smallSize = 1.0f
        // Para que toquen el piso, el centro debe estar a smallSize unidades sobre el piso
        float sphereRadius = 1.0f;
        float y            = floorY + sphereRadius; // Centro de la esfera justo a la altura para que toque el piso

        // Posición fija sin animación
        Instances[instanceIdx].Transform.SetTranslation(x, y, z);

        // Asignar textura basada en el índice
        Instances[instanceIdx].CustomId = i % NumTextures;
    }
    ...
    Attribs.InstanceCount = NumTotalInstances;
...
void Tutorial21_RayTracing::CreateSBT()
{
    ...
    // Bind the two glass spheres
    m_pSBT->BindHitGroupForInstance(m_pTLAS, "Glass Sphere 1", PRIMARY_RAY_INDEX, "GlassSpherePrimaryHit");
    m_pSBT->BindHitGroupForInstance(m_pTLAS, "Glass Sphere 2", PRIMARY_RAY_INDEX, "GlassSpherePrimaryHit");

    // Bind all the small spheres with standard shader
    for (int i = 0; i < NumSpheres - 2; i++)
    {
        // Use the same predefined names as in UpdateTLAS
        static const char* SphereNames[] = {
            "Small Sphere 1", "Small Sphere 2", "Small Sphere 3", "Small Sphere 4", "Small Sphere 5",
            "Small Sphere 6", "Small Sphere 7", "Small Sphere 8", "Small Sphere 9", "Small Sphere 10",
            "Small Sphere 11", "Small Sphere 12", "Small Sphere 13", "Small Sphere 14"};
        m_pSBT->BindHitGroupForInstance(m_pTLAS, SphereNames[i], PRIMARY_RAY_INDEX, "SpherePrimaryHit");
    }
    ...
    // Glass spheres
    m_pSBT->BindHitGroupForInstance(m_pTLAS, "Glass Sphere 1", SHADOW_RAY_INDEX, "SphereShadowHit");
    m_pSBT->BindHitGroupForInstance(m_pTLAS, "Glass Sphere 2", SHADOW_RAY_INDEX, "SphereShadowHit");

    // Small spheres
    for (int i = 0; i < NumSpheres - 2; i++)
    {
        // Use the same predefined names again for shadow hits
        static const char* SphereNames[] = {
            "Small Sphere 1", "Small Sphere 2", "Small Sphere 3", "Small Sphere 4", "Small Sphere 5",
            "Small Sphere 6", "Small Sphere 7", "Small Sphere 8", "Small Sphere 9", "Small Sphere 10",
            "Small Sphere 11", "Small Sphere 12", "Small Sphere 13", "Small Sphere 14"};
        m_pSBT->BindHitGroupForInstance(m_pTLAS, SphereNames[i], SHADOW_RAY_INDEX, "SphereShadowHit");
    }
...
void Tutorial21_RayTracing::Initialize(const SampleInitInfo& InitInfo)
{
    ...
    // Setup camera with wider view to see all spheres
    m_Camera.SetPos(float3(0.f, 0.0f, -25.0f));
